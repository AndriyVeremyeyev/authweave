...
import { InvalidWeightForm, parseWeightForm } from "../../../../../lib/assessment/weights.ts";
import { authConfiguration, sameOriginMutation } from "../../../../../lib/auth/config.ts";
import { previewPersonalWeightedComparison } from "../../../../../lib/auth/core-client.ts";
...
export async function POST(
  request: NextRequest, context: RouteContext<"/api/assessments/[id]/weighted-preview">,
): Promise<Response> {
  try {
    ...
    if (declaredLength && Number(declaredLength) > 2048) return noStore(413);
    const body = await request.text();
    if (body.length > 2048) return noStore(413);
    const { expectedVersion, weights } = parseWeightForm(new URLSearchParams(body));
    const result = await previewPersonalWeightedComparison(session, id, expectedVersion, weights);
    ...
    return new Response("Weighted preview is temporarily unavailable.", {
