...
import { InvalidEvaluationContextForm, parseEvaluationContextForm } from
  "../../../../../lib/assessment/evaluation-context.ts";
import { authConfiguration, sameOriginMutation } from "../../../../../lib/auth/config.ts";
import { updatePersonalEvaluationContext } from "../../../../../lib/auth/core-client.ts";
...
function returnToAssessment(origin: URL, id: string, error?: string): NextResponse {
  ...
  if (error) destination.searchParams.set("contextError", error);
...
export async function POST(
  request: NextRequest, context: RouteContext<"/api/assessments/[id]/evaluation-context">,
): Promise<Response> {
  try {
    ...
    const { expectedVersion, values } = parseEvaluationContextForm(new URLSearchParams(body));
    const result = await updatePersonalEvaluationContext(session, id, expectedVersion, values);
    ...
    if (error instanceof InvalidEvaluationContextForm) return noStore(400);
