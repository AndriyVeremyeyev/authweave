...
class CapabilityEvaluatorTests {
    ...
    void resultIsDeterministicAndDoesNotDependOnCatalogInputOrder() {
        ...
        assertEquals(CapabilityEvaluator.evaluate(profile, new ProviderCatalog(4, "test-1", Kind.SYNTHETIC, List.of(a, b)), NOW),
                CapabilityEvaluator.evaluate(profile, new ProviderCatalog(4, "test-1", Kind.SYNTHETIC, List.of(b, a)), NOW));
    }
    ...
    void factsDoNotLeakBetweenPlansOrRegions() {
        ...
        var results = CapabilityEvaluator.evaluate(profile(REQUIRED, NOT_REQUIRED),
                new ProviderCatalog(4, "test-1", Kind.SYNTHETIC, List.of(supported, missing)), NOW);
    ...
    void rejectsIncompleteProvenanceDuplicateOptionsAndRealSourcesInSyntheticCatalog() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new ProviderCatalog(4, "test", Kind.SYNTHETIC, List.of(option, option)));
        assertThrows(IllegalArgumentException.class, () -> new ProviderCatalog(5, "test", Kind.SYNTHETIC, List.of(option)));
    }
    ...
    private static Option option(String id, Map<Capability, Fact> facts) {
        return new Option(id, "Fictional option", "Synthetic plan", "Synthetic region", facts, Compatibility.empty());
    }

    private static ProviderCatalog catalog(Map<Capability, Fact> facts) {
        return new ProviderCatalog(4, "test-1", Kind.SYNTHETIC, List.of(option("a", facts)));
    }
