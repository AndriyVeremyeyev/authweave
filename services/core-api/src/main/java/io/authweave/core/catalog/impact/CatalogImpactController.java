...
@RestController
public final class CatalogImpactController {
    private final CatalogImpactService service;
    private final CatalogProposalRepository repository;
    private final ObjectMapper mapper;
    public CatalogImpactController(CatalogImpactService service, CatalogProposalRepository repository, ObjectMapper mapper) {
        this.service = service; this.repository = repository; this.mapper = mapper;
    }
    @PostMapping("/api/v1/catalog-change-proposals/impact-preview")
    public CatalogImpactPreview preview(@RequestBody CatalogChangePreviewRequest request) { return service.analyze(request); }

    @GetMapping("/api/v1/catalog-change-proposals/{proposalId}/revisions/{version}/impact-preview")
    public CatalogImpactPreview stored(@PathVariable UUID proposalId, @PathVariable @Min(0) @Max(9007199254740991L) long version) {
